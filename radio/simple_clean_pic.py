...
class Vis:
    ...
    def dirty(self,npix,du,vis=None):
        ...
        return dirty_map
    def observe_map(self,map,pixsize):
        assert(map.shape[0]==map.shape[1]) #because I am lazy
        du = 1/(map.shape[0]*pixsize)
        mapft=np.fft.rfft2(map)
        nvis=len(self.u)
        out=np.zeros(nvis,dtype='complex')
        ix=np.asarray(np.round(self.u/du),dtype='int')
        iy=np.asarray(np.round(self.v/du),dtype='int')
        nmax=map.shape[0]//2
        for i in range(nvis):
            if (ix[i]>=0)&(ix[i]<nmax)&(iy[i]>-nmax)&(iy[i]<nmax):
                out[i]=mapft[ix[i],iy[i]]
        return out
        
def clean(vis,npix,pixsize,fac=0.5,niter=20,plot=True):
    cat=SrcCat()
...
map=plt.imread('marion_team_saaii.jpg')
map=(map[:,:,0])
map=np.fft.fftshift(map[200:1400,:]).copy()
map=map/np.std(map)
npix=map.shape[0] #map size in pixels
fac=np.pi/180/60 #convert radians to arcmin
pixsize=0.2*fac #have 0.2' pixels

myvis=vis.observe_map(map,pixsize)
...
new_vis,new_cat=clean(vis,npix,pixsize,fac=0.5,niter=30*nsrc,plot=False)
