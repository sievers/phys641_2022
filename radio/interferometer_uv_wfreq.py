...
plt.title('Natural Weighting')

uv_uniform=1.0*uv_mat>0
plt.figure(3)
beam_uniform=np.abs(np.fft.ifft2(uv_uniform))
plt.figure(3)
plt.clf()
plt.imshow(np.fft.fftshift(beam_uniform))
plt.xlim([x0-dx,x0+dx])
plt.ylim([x0-dx,x0+dx])
